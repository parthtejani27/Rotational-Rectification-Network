...
interval_set_0_to_5 = 3  # Extract every third frame from set00 to set05
interval_set_6_to_10 = 30  
...
def extract_files(data_path, save_path, sets):
    ...
    for j, set_name in enumerate(sets):
        ...
        for i, video in enumerate(fnames):
            ...
            imgs, ext = read_seq(video_path)
            extracted_frames = range(0, len(imgs))
            if set_name in ['set00', 'set01', 'set02', 'set03', 'set04', 'set05']:
                extracted_frames = extracted_frames[::interval_set_0_to_5]
            elif set_name in ['set06', 'set07', 'set08', 'set09', 'set10']:
                extracted_frames = extracted_frames[::interval_set_6_to_10]
            for idx in extracted_frames:
                img = imgs[idx]
                img_fname = "I{}.{}".format(str(idx).zfill(5), ext)
                img_path = os.path.join(img_save_path, img_fname)
                with open(img_path, 'wb+') as f:
                    f.write(img)
